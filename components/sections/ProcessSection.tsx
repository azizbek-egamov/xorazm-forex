          ...
          {steps.map((step, index) => (
            <div key={index} className="relative h-full">
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 left-full w-12 h-0.5 bg-teal-500 z-0">
                  <ArrowRight className="absolute -right-2 -top-2 w-4 h-4 text-teal-500" />
                </div>
              )}

              <div
                className={`relative z-10 h-full flex flex-col p-6 lg:p-8 rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300 text-center ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"
                  }`}
              >
